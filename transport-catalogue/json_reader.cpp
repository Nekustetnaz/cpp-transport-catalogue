...
void JsonReader::PopulateCatalogue(transport_catalogue::TransportCatalogue& catalogue) {
    ...
    for (auto& request_stops : arr) {
        ...
        if (type == "Stop") {
            auto [stop_name, coordinates, stop_distances] = PopulateStop(request_stops_map);
    ...
    for (auto& request_bus : arr) {
        ...
        if (type == "Bus") {
            auto [bus_number, stops, is_roundtrip] = PopulateBus(request_bus_map);
            catalogue.AddBus(std::string{bus_number}, stops, is_roundtrip);
        }
    }
}

std::tuple<std::string_view, geo::Coordinates, std::map<std::string_view, int>> JsonReader::PopulateStop(const json::Dict& request_map) const {
    ...
    for (auto& request_stops: arr) {
        ...
        if (type == "Stop") {
            ...
            for (auto& [to_name, dist] : stop_distances) {
                catalogue.SetDistance(stop_name, to_name, dist);
            }
        }
    }
}

std::tuple<std::string_view, std::vector<std::string_view>, bool> JsonReader::PopulateBus(const json::Dict& request_map) const {
