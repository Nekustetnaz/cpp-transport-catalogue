...
std::vector<svg::Polyline> MapRenderer::GetRouteLines(const std::map<std::string_view, const transport_catalogue::Bus*>& buses, const SphereProjector& sp) const {
    ...
    for (const auto& [bus_number, bus] : buses) {
        ...
        if (color < (render_settings_.color_palette.size() - 1)) ++color;
        else color = 0;
...
std::vector<svg::Text> MapRenderer::GetBusName(const std::map<std::string_view, const transport_catalogue::Bus*>& buses, const SphereProjector& sp) const {
...
std::vector<svg::Circle> MapRenderer::GetStopMarks(std::map<std::string_view, const transport_catalogue::Stop*>& stops, const SphereProjector& sp) const {
...
std::vector<svg::Text> MapRenderer::GetStopNames(std::map<std::string_view, const transport_catalogue::Stop*>& stops, const SphereProjector& sp) const {
    std::vector<svg::Text> result;
    svg::Text text;
    svg::Text underlayer;
    for (const auto& [stop_name, stop] : stops) {
...
svg::Document MapRenderer::GetSVG(const std::vector<const transport_catalogue::Stop*>& stops, const std::map<std::string_view, const transport_catalogue::Bus*>& buses) const {
    svg::Document result;
    std::vector<geo::Coordinates> stops_coord;
    std::map<std::string_view, const transport_catalogue::Stop*> sorted_stops;

    for (const auto& stop : stops) {
        stops_coord.push_back(stop->coordinates);
        sorted_stops.insert({stop->name, stop});
    }
    SphereProjector sp(stops_coord.begin(), stops_coord.end(), render_settings_.width, render_settings_.height, render_settings_.padding);
    for (const auto& line : GetRouteLines(buses, sp)) {
        result.Add(line);
    }
    for (const auto& bus_name : GetBusName(buses, sp)) {
        result.Add(bus_name);
    }
    for (const auto& stop_mark : GetStopMarks(sorted_stops, sp)) {
        result.Add(stop_mark);
    }
    for (const auto& stop_name : GetStopNames(sorted_stops, sp)) {
